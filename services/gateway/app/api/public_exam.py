...
async def start_exam(token: str, request: StartExamRequest) -> dict:
    try:
        return gateway_demo_store.start_exam(token, request.verification_code)
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
async def save_answer(token: str, question_id: str, request: SaveAnswerRequest) -> dict:
    try:
        return gateway_demo_store.save_answer(token, question_id, request.draft_answer)
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
...
async def create_risk_event(token: str, request: RiskEventRequest) -> dict:
    try:
        return gateway_demo_store.record_risk_event(token, request.event_type, request.payload)
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
...
async def run_coding_case(token: str, request: RunCodingRequest) -> dict:
    try:
        gateway_demo_store._require_active_session(token)
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
...
async def submit_coding_case(token: str, request: SubmitCodingRequest) -> dict:
    try:
        question = gateway_demo_store.get_coding_question(token, request.question_id)
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
